...
$python -m src.extract_log -m ${max_rk} "$DATASET_DIR/queries_multi.tsv" \
  "$DATASET_DIR/clicks_multi.tsv" "${feat_dir}/train.feat.txt" $log_dir

$python -m src.arxiv_process -m ${max_rk} -d ${dim} train ${log_dir} ${feat_dir} ${npy_dir}

# === mlp without relevance ==
echo 'Estimating without relevance model...'
model_dir="${res_dir}/mlp"
mkdir -p ${model_dir}
$python -m src.model.ann_arxiv -m ${max_rk} -d ${dim} -n1 1 \
...
mkdir -p ${model_dir}
$python -m src.model.ann_arxiv -m ${max_rk} -d ${dim} -n1 1 -n2 1 \
  mlp ${npy_dir} ${model_dir}
