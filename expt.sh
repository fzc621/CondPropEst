...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
  python="/home/zf94/anaconda3/bin/python"
else
...
ground_truth_dir="${expt_dir}/ground_truth"
...
$python -m src.sim_feat "${DATA_DIR}/set1bin.train.txt" $DATA_DIR
...
for i in 0 1;
do
  $python -m src.sim_click -s $sw -m power -d 10 -w $w "${ground_truth_dir}/para.dat" \
    "${DATA_DIR}/set1bin.train.txt" "${expt_dir}/train.score${i}.dat" \
    "${DATA_DIR}/set1bin.train.feat.txt" "${log_dir}/train.log${i}.txt"
done

$python -m src.data_process -m 10 -d 10 train ${log_dir} ${DATA_DIR} ${NPY_DIR}
rm -rf ${DATA_DIR}
